...
class GuardServiceTest {
    ...
    private static final GameProfile GAME_PROFILE =
        new GameProfile(UUID.fromString("00000000-0000-0000-0000-000000000001"), "TestPlayer");
    ...
    void shouldBypass_opPlayer_returnsTrue() {
        try (MockedStatic<LuckPermsCompat> lp = mockStatic(LuckPermsCompat.class)) {
            lp.when(LuckPermsCompat::isAvailable).thenReturn(false);
            when(server.getProfilePermissions(any())).thenReturn(2); // niveau OP
    ...
    void shouldBypass_nonOpPlayer_returnsFalse() {
        try (MockedStatic<LuckPermsCompat> lp = mockStatic(LuckPermsCompat.class)) {
            lp.when(LuckPermsCompat::isAvailable).thenReturn(false);
            when(server.getProfilePermissions(any())).thenReturn(0); // pas OP
    ...
    void shouldBypass_opLevelBelowThreshold_returnsFalse() {
        // BYPASS_OP_LEVEL=2 → niveau 1 ne doit pas suffire
        try (MockedStatic<LuckPermsCompat> lp = mockStatic(LuckPermsCompat.class)) {
            lp.when(LuckPermsCompat::isAvailable).thenReturn(false);
    ...
    void shouldBypass_thirdPartyModCannotGrantBypass() {
        ...
        // La correction : on lit ops.json via getProfilePermissions(), pas hasPermissions().
        try (MockedStatic<LuckPermsCompat> lp = mockStatic(LuckPermsCompat.class)) {
            lp.when(LuckPermsCompat::isAvailable).thenReturn(false);
            // ops.json = niveau 0 (pas OP)
            when(server.getProfilePermissions(any())).thenReturn(0);
            // hasPermissions() ne doit plus être appelé
    ...
    // --- Exception dans getProfilePermissions → fail-safe retourne false ---
    ...
    void shouldBypass_luckPermsGrantsBypass_returnsTrue() {
        ...
        try (MockedStatic<LuckPermsCompat> lp = mockStatic(LuckPermsCompat.class)) {
            ...
            assertTrue(guardService.shouldBypass(player));
            // getProfilePermissions ne doit PAS être appelé si LuckPerms accorde le bypass
